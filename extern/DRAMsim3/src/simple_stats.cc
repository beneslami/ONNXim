...
template <class T>
void PrintStatText(std::ostream& where, std::string name, T value, std::string description) {
    // not making this a class method because we need to calculate
    // power & bw later, which are not BaseStat members
    where << fmt::format("{:<30}{:^3}{:>12}{:>5}{}", name, " = ", value, " # ", description) << std::endl;
    return;
}

SimpleStats::SimpleStats(const Config& config, int channel_id) : config_(config), channel_id_(channel_id) {
    // counter stats
...
void SimpleStats::InitStat(std::string name, std::string stat_type, std::string description) {
...
void SimpleStats::InitVecStat(std::string name, std::string stat_type, std::string description, std::string part_name, int vec_len) {
...
double SimpleStats::GetHistoAvg(const HistoCount& hist_counts) const {
    ...
    return count == 0 ? 0.0 : static_cast<double>(accu_sum) / static_cast<double>(count);
}
