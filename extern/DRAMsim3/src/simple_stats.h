...
class SimpleStats {
   public:
    ...
    double GetCalcStat(const std::string& name) const {
        auto it = calculated_.find(name);
        return (it != calculated_.end()) ? it->second : 0.0;
    }
   ...
   private:
    ...
    void InitVecStat(std::string name, std::string stat_type, std::string description, std::string part_name, int vec_len);
    void InitHistoStat(std::string name, std::string description, int start_val, int end_val, int num_bins);
